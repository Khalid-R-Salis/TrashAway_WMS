...
const Landing = () => {
  ...
  const handlePickupRequest = async (e) => {
    ...
    try {
      ...
      // Check and log token and userId separately
      if (token) {
      ...
      const response = await api.post(
        `api/user/request-pickup/${userId}`,
        {
          capacity,
          location,
          time,
      ...
      >
        ...
        <div className=" flex flex-col justify-center items-center gap-[16px] pt-[150px] font-Inter">
          ...
          <p className=" w-[60rem] text-center text-[20px] text-gray-black">
            Explore the significant strides we've made in waste management, from
            reducing pollution to enhancing community health and sustainability
            across the state.
          </p>
        </div>
